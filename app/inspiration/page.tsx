...
export default function InspirationsPage() {
  ...
  useEffect(() => {
    const fetchDesigns = async () => {
      try {
        const res = await fetch('http://localhost:5000/api/inspiration');
        const data = await res.json();

        // TRANSFORM: Convert Backend Data -> Frontend Format
        const mappedDesigns = data.map((item: any) => ({
          id: item.id,
          title: item.title,
  ...
  // --- LIKE FUNCTION (Optimistic + Backend Save) ---
  const toggleLike = async (id: number) => {
    ...
    // 2. Server Update (Save it to the database permanently)
    try {
      await fetch(`http://localhost:5000/api/inspiration/${id}/like`, { 
        method: 'POST' 
      });
      console.log(`Saved like for Design ${id}`);
    } catch (error) {
  ...
  return (
    <main 
      ...
      style={{
          ['--font-body' as any]: 'var(--font-poppins)',
          ['--font-heading' as any]: 'var(--font-poppins)',
      ...
      <div className="max-w-[1400px] mx-auto px-6 py-10">
        ...
        {!selectedDesign && (
          <>
            ...
            {/* GRID LAYOUT */}
            {isLoading ? (
                <div className="flex justify-center items-center py-20 text-gray-400">Loading Inspiration...</div>
